                         ...
                         uint32_t count )
{
  ...
  {
    is_equal &= dataA[i] >= ( dataB[i] - EPSILON_F ) && dataA[i] <= ( dataB[i] + EPSILON_F );
  }
  return is_equal;
}
...
static struct Quat normalize_quat( struct Quat q )
{
  ...
  struct Quat norm_q = {  .i = q.i / denom,
                          .j = q.j / denom,
                          .k = q.k / denom,
                          .real = q.real / denom, };
                             ...
                             float radian_roll )
{
  ...
  return ( struct Quat ){ cos_y * sin_r * cos_p - sin_y * cos_r * sin_p,
...
struct Vec3f quat_to_euler( struct Quat q )
{
  ...
  {
    return ( struct Vec3f ){(float)M_PI / 2.f, 2.f * atan2f( q.x, q.w ), 0.f};
  }
  ...
  return ( struct Vec3f ){atan2f( 2.f * ( q.w * q.x + q.y * q.z ), 1.f - 2.f * ( q.x * q.x + q.y * q.y ) ),
                          asinf( 2.f * ( q.w * q.y - q.z * q.x ) ),
                          atan2f( 2.f * ( q.w * q.z + q.x * q.y ), 1.f - 2.f * ( q.y * q.y + q.z * q.z ) )};
}
...
{
  struct Quat q = { .x = lhs.w * rhs.x + lhs.x * rhs.w + lhs.y * rhs.z - lhs.z * rhs.y,
...
struct Vec3f mult_qv( struct Quat q, struct Vec3f v )
{
  ...
  return add_v3f( v, mult_v3f( add_v3f( mult_v3f( t_vec, q.w ), t2_vec ), 2.f ) );
}
...
float determinant_3x3( struct Mat3x3 m )
{
  return m.data[0][0] * ( m.data[1][1] * m.data[2][2] - m.data[2][1] * m.data[1][2] ) +
         m.data[1][0] * ( m.data[2][1] * m.data[0][2] - m.data[0][1] * m.data[2][2] ) +
...
float determinant_4x4( struct Mat4x4 m )
{
  ...
  return m.data[0][0] * val_00 + m.data[0][1] * val_10 + m.data[0][2] * val_20 + m.data[0][3] * val_30;
}
...
struct Mat3x3 inverse_3x3( struct Mat3x3 m )
{
  ...
  struct Mat3x3 adjoint = { .data = { { m.data[1][1] * m.data[2][2] - m.data[2][1] * m.data[1][2],
...
static bool equals_mat_impl( const float dataA[], const float dataB[], uint32_t row, uint32_t column )
{
  ...
  for( uint32_t i = 0; i < column && is_equal; i++ )
  {
    for( uint32_t j = 0; j < row && is_equal; j++ )
    {
      ...
      is_equal &= dataA[idx] >= ( dataB[idx] - EPSILON_F ) && dataA[idx] <= ( dataB[idx] + EPSILON_F );
    }
  }
...
static void transpose_impl( float output[], const float input[], uint32_t row, uint32_t col )
{
  // data is stored in column major order
...
static void scalar_mult_impl( float scalar, float input[], uint32_t row, uint32_t col )
{
  // data is stored in column major order
...
{
  return ( struct Mat4x4 ){{  {m.data[0][0],
                               m.data[0][1],
							   m.data[0][2],
							   m.data[0][3] },
                              {m.data[1][0],
...
{
  return ( struct Mat4x4 ){{  {m.data[0][0] * v.x,
                               m.data[0][1],
							   m.data[0][2],
							   m.data[0][3] },
                              {m.data[1][0],
...
{
  return ( struct Mat3x3 ){{  {m.data[0][0],
                               m.data[0][1],
							   m.data[0][2]},
                              {m.data[1][0],
							   m.data[1][1],
  ...
  float fovy, float scr_width, float scr_height, float near, float far )
{
  ...
  float yscale = cosf( half_fov ) / sinf( half_fov );
