...
int main( int argc, char const* argv[] )
{
  ...
  struct Vec3f test_vectors[] = { {	.x = 1.f}, 
                                  { .y = 0.2f, 
                                    .z = 0.8f}, 
                                  {	.x = 0.1f, 
                                    .y = 0.4f, 
  ...
  const uint32_t test_vectors_count = sizeof( test_vectors ) / sizeof( struct Vec3f );

  for( uint32_t i = 0; i < test_vectors_count; i++ )
  {
    printf( "-- Test #%u:: %s\n", 
            i, 
            stringify_v3f( test_vectors[i] ).buffer );
  ...
  printf( "-- Test #5 (subtraction)\n%s\n",
          stringify_m4x4( add_4x4( ident_m4x4(), mult_4x4( ident_m4x4(), -5.f ) ) ).buffer );
